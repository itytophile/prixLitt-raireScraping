            ...
            if len(liens) > 1:
                ...
                if not t.isdigit():
                    if t not in liste_auteur:
                        liste_auteur += [t]
                        t = (
                            urls_auteur.index(url),
                            liens[0].text,
                            liste_auteur.index(t),
                        )
        ...
        if len(cellules) > 2:
            ...
            for i in cellules[3].xpath("./i/a"):
                if i.text not in liste_oeuvre:
                    liste_oeuvre += [i.text]
                    t = (
                        urls_oeuvre.index(url),
                        cellules[0].xpath("./a")[0].text,
                        liste_oeuvre.index(i.text),
                    )
